...
# Save region metadata in csv file
def save_download(city, region, country, path, date, link):
    try:
        exist = False
        with open('data/regions-metadata.csv', 'r', newline='', encoding='UTF8') as r:
            csvreader = csv.reader(r, delimiter=',')
            for row in csvreader:
                if row[3] == path:
                    exist = True
        if not exist:
            with open('data/regions-metadata.csv', 'a', newline='', encoding='UTF8') as w:
                csvwriter = csv.writer(w, delimiter=',')
                csvwriter.writerow([country, region, city, path, date, link])
    except:
        with open('data/regions-metadata.csv', 'a', newline='', encoding='UTF8') as w:
                csvwriter = csv.writer(w, delimiter=',')
                csvwriter.writerow([country, region, city, path, date, link])
